...
export async function POST(req: Request) {
  try {
    ...
    if (user.addresses.length === 0) {
      ...
      user.addresses.forEach((a: IAddressDoc) => {
        a.isDefault = false;
      });
    }
...
export async function PATCH(req: Request) {
  try {
    ...
    const addressIndex = user.addresses.findIndex(
      (a: IAddressDoc) => a._id.toString() === id,
    );
    if (addressIndex === -1)
...
export async function DELETE(req: Request) {
  try {
    ...
    user.addresses = user.addresses.filter((a: IAddressDoc) => a._id.toString() !== id);

    if (user.addresses.length > 0 && !user.addresses.some((a: IAddressDoc) => a.isDefault)) {
