...
export default function MyRequestsPage() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
      {/* Header */}
      <div className="pb-6 border-b border-slate-200 dark:border-[#1f1f1f] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            <GitPullRequestDraft className="w-6 h-6 text-blue-500" />
            My Requests
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Track the status of members you have requested to add to the family tree.
          </p>
        </div>
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-[#1f1f1f] border border-slate-200 dark:border-[#303030] text-sm font-semibold text-slate-700 dark:text-slate-300">
      ...
      {/* Requests List */}
      {requests.length === 0 ? (
        <div className="bg-white dark:bg-[#141414] rounded-2xl border border-slate-200 dark:border-[#1f1f1f] shadow-sm p-12 flex flex-col items-center text-center">
          <div className="w-16 h-16 bg-slate-50 dark:bg-[#1f1f1f] rounded-full flex items-center justify-center mb-4">
            <GitPullRequestDraft className="w-8 h-8 text-slate-300 dark:text-slate-600" />
          </div>
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">No requests found</h3>
          <p className="text-sm text-slate-500 mt-2 max-w-md">
            You haven&apos;t submitted any family tree addition requests yet. When you add a new member from the Family Tree, their status will appear here.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {requests.map((req) => (
            <div 
              key={String(req._id)} 
              className="bg-white dark:bg-[#141414] rounded-2xl border border-slate-200 dark:border-[#1f1f1f] shadow-sm p-5 hover:shadow-md transition-shadow relative overflow-hidden"
            >
              {/* Status Badge */}
