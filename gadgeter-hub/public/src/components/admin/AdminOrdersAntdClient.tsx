...
import { CarrierSelectorModal } from "./CarrierSelectorModal";
import { syncOrderCarrierStatusAction } from "../../actions/courierActions";

import { AdminOrderMobileCard } from "./AdminOrderMobileCard";
...
export function AdminOrdersAntdClient({
  ...
  const [selectedShipOrder, setSelectedShipOrder] =
    useState<IOrderSerializable | null>(null);
  ...
  const [editConsignmentModalOpen, setEditConsignmentModalOpen] =
    useState(false);
  ...
  const openEditConsignmentModal = (
    orderId: string,
    orderNum: string,
    currentId?: string,
  ) => {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    const matchesChannel =
      channelFilter === "all" ||
      (order.channelSource || "web") === channelFilter;

    const matchesCarrier =
      carrierFilter === "all" ||
      (order.courierProvider || "offline") === carrierFilter;

    return matchesSearch && matchesStatus && matchesChannel && matchesCarrier;
  });

  const activeCount = orders.filter(
    (o) => !["returned", "cancelled"].includes(o.orderStatus || ""),
  ).length;
  const inReturnCount = orders.filter(
    (o) => o.orderStatus === "in_return",
  ).length;
  ...
  const pathaoCount = orders.filter(
    (o) => o.courierProvider === "pathao",
  ).length;
  const steadfastCount = orders.filter(
    (o) => o.courierProvider === "steadfast",
  ).length;
  const offlineCount = orders.filter(
    (o) => !o.courierProvider || o.courierProvider === "offline",
  ).length;
  ...
  const columns: ColumnsType<IOrder> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        const vipDeduction =
          record.vipPrivilege && record.vipPrivilege > 0
            ? record.vipPrivilege
            : record.discount || 0;
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <Text
            strong
            style={{ fontSize: "14px", color: "#0f172a", display: "block" }}
          >
          ...
          {Boolean(record.advancePaid && record.advancePaid > 0) && (
            <Text
              type="secondary"
              style={{ fontSize: "10px", color: "#2563eb", fontWeight: 700 }}
            >
            ...
            {record.courierStatus && (
              <div style={{ marginTop: 2 }}>
                <Tag
                  ...
                  onClick={() =>
                    trackingId &&
                    openTrackingModal(trackingId, record.orderNumber)
                  }
                >
                  {courierInfo.label}
                </Tag>
    ...
    <div style={{ padding: 0 }} className="space-y-4">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (pendingIds.length === 0)
                    return toast.error("কোনো পেন্ডিং অর্ডার নেই");

                  // Open first pending invoice or batch window
                  window.open(
                    `/admin/orders/${pendingIds[0]}/invoice`,
                    "_blank",
                  );
                }}
                style={{ fontWeight: 700 }}
              ...
              >
                ...
                <Select
                  ...
                  options={[
                    ...
                    {
                      value: "steadfast",
                      label: `🦅 Steadfast (${steadfastCount})`,
                    },
      ...
      <div className="hidden lg:block">
        ...
        >
          <Table
            ...
            rowClassName={(record) =>
              record.orderStatus === "in_return"
                ? "in-return-table-row font-semibold"
                : ""
            }
            scroll={{ x: 800 }}
            pagination={
              isDashboard ? false : { pageSize: 10, showSizeChanger: true }
            }
            locale={{
              emptyText: (
      ...
      <Modal
        ...
        footer={[
          <Button
            key="cancel"
            onClick={() => setEditConsignmentModalOpen(false)}
      ...
      >
        <div className="space-y-3">
          <Text type="secondary" style={{ fontSize: "12px", display: "block" }}>
            পাঠাও হাব থেকে বুকিং করার পর পাওয়া Consignment ID (যেমন:{" "}
            <Text code>SG030826FV2JHW</Text>) নিচে লিখুন। ভুল বসানো হলে ঘরটি
            খালি রেখে **Save & Sync** অথবা **Remove ID** বাটনে চাপুন।
          </Text>
          <Input
