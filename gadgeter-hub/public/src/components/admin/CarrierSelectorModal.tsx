...
export const CarrierSelectorModal: React.FC<CarrierSelectorModalProps> = ({
  ...
  const [activeCarrier, setActiveCarrier] =
    useState<CourierProviderName>("pathao");
  ...
  useEffect(() => {
    if (visible && order) {
      ...
      form.setFieldsValue({
        ...
        storeId: order.courierStoreId
          ? Number(order.courierStoreId)
          : undefined,
      });
  ...
  const handleBook = async () => {
    try {
      ...
      if (result.success && result.order) {
        message.success(
          result.message ||
            `${activeCarrier.toUpperCase()} এ অর্ডার বুকিং সম্পন্ন হয়েছে!`,
        );
      ...
      <Form form={form} layout="vertical" style={{ marginTop: 12 }}>
        {activeCarrier === "pathao" && (
          <>
            ...
            <Form.Item
              ...
              label={
                <span>
                  <ShopOutlined style={{ marginRight: 4 }} /> পিকআপ মার্চেন্ট
                  স্টোর (Store ID)
                </span>
              }
              rules={[
                {
                  required: true,
                  message: "দয়া করে পাঠাও স্টোর নির্বাচন করুন",
                },
              ]}
            >
              <Select
            ...
            <Form.Item
              name="note"
              label="বিশেষ নির্দেশনাবলী (Special Instructions)"
            >
              <Input.TextArea
                rows={2}
        ...
        {activeCarrier === "steadfast" && (
          <>
            ...
            <Form.Item
              name="note"
              label="ডেলিভারি নোট (Delivery Note / Instruction)"
            >
              <Input.TextArea
                rows={2}
