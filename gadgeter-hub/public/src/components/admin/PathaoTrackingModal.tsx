  ...
  const fetchTracking = async (cid: string) => {
    ...
    try {
      ...
      if (!proxyData.success) {
        setResult({
          success: false,
          error: proxyData.error || "Failed to fetch tracking",
        });
        return;
      }
      ...
      if (json.code !== 200 || !json.data || !json.data.order) {
        setResult({
          success: false,
          error: json.message || "Invalid Consignment ID or no data found",
        });
        return;
      }
      ...
      const stateName =
        json.data.state?.name || orderData.transfer_status || "Unknown";
      ...
      for (const logItem of logs) {
        ...
        if (
          logItem.desc &&
          logItem.desc.toLowerCase().includes("assigned to")
        ) {
        ...
        >
          ...
          {consignmentId && (
            <Button
              size="small"
              icon={
                <ReloadOutlined spin={loading} style={{ color: "#e11d48" }} />
              }
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/90 flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xs">
            ...
            {(() => {
              ...
              for (const l of logs) {
                ...
                if (
                  gStatus === "Ready For Delivery" ||
                  l.desc?.toLowerCase().includes("assigned to")
                ) {
                  attempts++;
                }
              }
              ...
              <div>
                ...
                >
                  {trackingData.riderName
                    ? `Assigned Hero: ${trackingData.riderName}`
                    : "A rider has been assigned to deliver this parcel."}{" "}
                  (Call customer to ensure availability)
                </Text>
              </div>
              ...
              <Timeline
                ...
                items={logs.map((item, index) => {
                  ...
                  if (
                    ...
                    descLower.includes("hold") ||
                    notesLower.includes("hold") ||
                    notesLower.includes("reason:")
                  ) {
                    color = "orange";
                    dotIcon = (
                      <AlertOutlined
                    ...
                    descLower.includes("assigned")
                  ) {
                    color = "blue";
                    dotIcon = (
                      <CarOutlined style={{ fontSize: 14, color: "#2563eb" }} />
                    );
                  }
                      ...
                      <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-100 hover:border-slate-200 transition-all mb-1">
                        ...
                        {item.notes && (
                          <Text
                            ...
                            style={{
                              fontSize: "11px",
                              color: notesLower.includes("reason:")
                                ? "#b45309"
                                : "#475569",
                              fontWeight: notesLower.includes("reason:")
                                ? 700
                                : 400,
                              display: "block",
                              marginTop: 4,
