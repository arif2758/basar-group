...
export function CreateOrderModal({
  ...
  const [deliveryArea, setDeliveryArea] = useState<DeliveryZone | "custom">(
    "outside",
  );
  ...
  const handleAiParse = async () => {
    if (!aiText.trim()) {
      toast.error("মেসেজ বা চ্যাট টেক্সট পেস্ট করুন");
      return;
    }
    setIsParsing(true);
    try {
      ...
      if (Array.isArray(data.items) && data.items.length > 0) {
        const parsedOrderItems: AdminOrderItem[] = data.items.map((it: any) => {
          const prod =
            productList.find((p) => p._id === it.productId) ||
            getDefaultProduct(productList) ||
            productList[0];
          return {
            productId: prod?._id || "",
      ...
      toast.success(
        "AI পার্সিং সফল হয়েছে! তথ্যাদি ম্যানুয়াল ফর্মে লোড হয়েছে।",
      );
      setActiveTab("manual");
    } catch (err: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await createAdminManualOrder({
        ...
        deliveryArea,
        customShippingCost:
          deliveryArea === "custom" ? customShippingCost : undefined,
        paymentMethod,
        paymentProvider:
          paymentMethod === "mobile" ? paymentProvider : undefined,
  ...
  const manualFormContent = (
    <div className="space-y-4 pt-1 max-h-[70vh] overflow-y-auto pr-2">
      ...
      <div className="space-y-2">
        <Flex align="center" justify="space-between">
          <Text strong style={{ fontSize: "12px" }}>
            <ShoppingCartOutlined style={{ color: "#1677ff" }} /> ৩.
            প্রোডাক্টসমূহ ({orderItems.length})
          </Text>
          <Button
            size="small"
            type="dashed"
                ...
                <div className="space-y-2.5">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleUpdateItem(idx, "productTitle", e.target.value)
                      }
                      style={{
                        borderRadius: 8,
                        fontSize: "12px",
                  ...
                  <Flex align="center" gap={6} wrap="wrap">
                    ...
                    <Input
                      ...
                      value={item.color || ""}
                      onChange={(e) =>
                        handleUpdateItem(idx, "color", e.target.value)
                      }
                      style={{
                        width: 120,
                        height: 28,
                  ...
                  <Flex
                    align="center"
                    gap={10}
                    className="w-full justify-between pt-2 border-t border-slate-200/60"
                  >
      ...
      <div className="space-y-2">
        <Flex align="center" justify="space-between">
          <Text strong style={{ fontSize: "12px" }}>
            <EnvironmentOutlined style={{ color: "#1677ff" }} /> ডেলিভারি ঠিকানা
          </Text>
          <Text type="secondary" style={{ fontSize: "11px", fontWeight: 700 }}>
            ওজন: {totalWeightGrams}g
          </Text>
        </Flex>
        <div className="grid sm:grid-cols-3 gap-2">
          <Input
            placeholder="এরিয়া / গ্রাম *"
            value={addressLine1}
      ...
      {/* Summary Footer */}
      <Card
        style={{ background: "#0f172a", borderRadius: 16, marginTop: 24 }}
      ...
      >
        <div style={{ textAlign: "center" }}>
          ...
          <Title
            level={3}
            style={{ color: "#ffffff", margin: "0 0 4px", fontWeight: 900 }}
          >
          ...
          >
            {advancePaid > 0
              ? `(COD কালেকশন মোট: ${formatPrice(codAmount)})`
              : "(ক্যাশ অন ডেলিভারি মোট)"}
          </Text>
          <Button
            type="primary"
            size="large"
                  ...
                  <div className="space-y-4 pt-1">
                    <div>
                      ...
                      <Select
                        value={channelSource}
                        onChange={(val) =>
                          setChannelSource(val as ChannelSource)
                        }
                        style={{ width: "100%", height: 42 }}
