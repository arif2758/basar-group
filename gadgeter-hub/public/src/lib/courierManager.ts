...
export async function bookOrderWithCarrier(
  ...
  const fullAddress = addressParts.join(", ") || "Dhaka, Bangladesh";
  ...
  if (provider === "pathao") {
    const pathaoRes = await createPathaoOrder({
      ...
      item_quantity:
        order.items?.reduce((acc, i) => acc + i.itemQuantity, 0) || 1,
      item_weight: options.weightKg || 0.5,
      item_description: itemDesc.slice(0, 200),
      special_instruction: (options.note || order.customerNotes || "").slice(
        0,
        200,
      ),
  ...
  if (provider === "steadfast") {
    if (!consignmentId && !order.orderNumber) {
      return {
        success: false,
        provider,
        error: "No Steadfast Consignment ID or Invoice",
      };
    }
