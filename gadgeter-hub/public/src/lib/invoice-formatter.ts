...
export function buildInvoiceText(
  ...
  const isOutside =
    rawAddrParts.some((p) => /outside dhaka/i.test(p)) ||
    order.shippingCost > 80;
  const zoneLabel =
    addr.deliveryZone ||
    (isOutside ? "OSD (Outside Dhaka)" : "ISD (Inside Dhaka)");

  const cleanAddrParts = rawAddrParts.filter(
    (p) => !/suburbs|suburb|outside dhaka|inside dhaka|^dhaka$/i.test(p.trim()),
  );
  const cleanAddrStr =
    (cleanAddrParts.length > 0
      ? cleanAddrParts.join(", ")
      : addr.addressLine1) + (addr.postalCode ? ` - ${addr.postalCode}` : "");
    ...
    .map((item, index) => {
      ...
      if (variantInfo) {
        ...
        if (maxTitleLen > 3 && item.productTitle.length > maxTitleLen) {
          displayTitle =
            item.productTitle.slice(0, maxTitleLen - 1) + "…" + variantSuffix;
        } else {
