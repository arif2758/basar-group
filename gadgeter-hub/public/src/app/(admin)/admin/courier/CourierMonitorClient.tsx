...
import FollowUpPanel, {
  OUTCOME_CONFIG,
} from "../../../../components/admin/FollowUpPanel";
    ...
    >
      ...
      {last.note && (
        <Text type="secondary" style={{ fontSize: 10, fontStyle: "italic" }}>
          &ldquo;{last.note.slice(0, 22)}
          {last.note.length > 22 ? "…" : ""}&rdquo;
        </Text>
      )}
    </div>
  );
...
export function CourierMonitorClient({ initialOrders }: Props) {
  ...
  const isOrderAssigned = (o: IOrderSerializable) => {
    ...
    return (
      ...
      (statusLower.includes("ready") &&
        Boolean(o.courierRiderPhone || o.courierRiderName))
    );
  };
  ...
  const handleFollowUpAdded = (
    orderId: string,
    newEntry: import("../../../../types/order").IFollowUpEntry,
  ) => {
    setOrders((prev) =>
      prev.map((o) =>
        o._id === orderId
  ...
  const filtered = orders
    .filter((o) => {
      ...
      const trackingMatch = (o.courierTrackingId || "")
        .toLowerCase()
        .includes(q);
      const matchesSearch =
        !q || nameMatch || phoneMatch || orderNumMatch || trackingMatch;
      ...
      if (activeTab === "on_hold")
        return statusLower.includes("hold") && !isOrderAssigned(o);
      if (activeTab === "ready")
        return statusLower.includes("ready") && !isOrderAssigned(o);
      if (activeTab === "in_transit")
        return (
          (statusLower.includes("transit") || statusLower.includes("picked")) &&
          !isOrderAssigned(o)
        );
    ...
    .sort((a, b) => {
      const aDelivered = (a.courierStatus || "")
        .toLowerCase()
        .includes("delivered");
      const bDelivered = (b.courierStatus || "")
        .toLowerCase()
        .includes("delivered");
  ...
  const onHoldCount = orders.filter((o) =>
    (o.courierStatus || "").toLowerCase().includes("hold"),
  ).length;
  const readyCount = orders.filter((o) =>
    (o.courierStatus || "").toLowerCase().includes("ready"),
  ).length;
  ...
  const returnedCount = orders.filter((o) =>
    (o.courierStatus || "").toLowerCase().includes("return"),
  ).length;
  const deliveredCount = orders.filter((o) =>
    (o.courierStatus || "").toLowerCase().includes("delivered"),
  ).length;
  ...
  const columns: ColumnsType<IOrderSerializable> = [
        ...
        return (
          <div className="space-y-1">
            <Link href={`/admin/orders/${record._id}`}>
              <Text
                code
                style={{ fontWeight: 800, fontSize: "13px", color: "#1677ff" }}
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const trackingId =
          record.courierTrackingId || record.courierConsignmentId || "";
          ...
          <div className="space-y-1.5" style={{ maxWidth: 320 }}>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {lastUpdated && (
                ...
                >
                  <ClockCircleOutlined
                    style={{ fontSize: "9px", marginRight: 2 }}
                  />
                  {lastUpdated}
                </Text>
              )}
    ...
    },
    {
      ...
      render: (_, record) => {
        ...
        const cleanRider = (record.courierRiderPhone || "").replace(
          /[^0-9]/g,
          "",
        );
        const waRider = cleanRider.startsWith("88")
          ? cleanRider
          : `88${cleanRider}`;
        return (
          <div className="space-y-1">
        ...
        return (
          <Flex vertical gap={6} align="center">
            <Button
              ...
              onClick={() =>
                openTrackingModal(record.courierTrackingId!, record.orderNumber)
              }
            >
              Track Live
            </Button>
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
        <div>
          ...
          <Text type="secondary" style={{ fontSize: "13px" }}>
            সরাসরি পাঠাও কোরিয়ারের পার্সেল ট্র্যাকিং, অন-হোল্ড নোটিশ ও রাইডার
            কনট্যাক্ট ম্যানেজমেন্ট
          </Text>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <Card
          ...
          className="text-center bg-indigo-50/60 border-indigo-200"
        >
          <Statistic
            title={
              <Text
                style={{ fontSize: "11px", fontWeight: 700, color: "#4338ca" }}
              >
                🎯 ASSIGNED
              </Text>
            }
            value={assignedCount}
            styles={{
              content: { fontWeight: 900, fontSize: "22px", color: "#4f46e5" },
            }}
          />
        </Card>
        ...
        <Card
          style={{ borderRadius: 16 }}
          styles={{ body: { padding: 14 } }}
          className="text-center bg-amber-50/50 border-amber-200"
        >
          <Statistic
            title={
              <Text
            ...
            value={onHoldCount}
            styles={{
              content: { fontWeight: 900, fontSize: "22px", color: "#d97706" },
            }}
          />
        </Card>
        ...
        <Card
          style={{ borderRadius: 16 }}
          styles={{ body: { padding: 14 } }}
          className="text-center bg-blue-50/50 border-blue-200"
        >
          <Statistic
            title={
              <Text
            ...
            value={readyCount}
            styles={{
              content: { fontWeight: 900, fontSize: "22px", color: "#2563eb" },
            }}
          />
        </Card>
        ...
        >
          <Statistic
            ...
            value={transitCount}
            styles={{
              content: { fontWeight: 900, fontSize: "22px", color: "#0284c7" },
            }}
          />
        </Card>
        ...
        <Card
          style={{ borderRadius: 16 }}
          styles={{ body: { padding: 14 } }}
          className="text-center bg-red-50/50 border-red-200"
        >
          <Statistic
            title={
              <Text
                style={{ fontSize: "11px", fontWeight: 700, color: "#b91c1c" }}
              >
                🚨 RETURNED
              </Text>
            }
            value={returnedCount}
            styles={{
              content: { fontWeight: 900, fontSize: "22px", color: "#dc2626" },
            }}
          />
        </Card>
        ...
        <Card
          style={{ borderRadius: 16 }}
          styles={{ body: { padding: 14 } }}
          className="text-center bg-emerald-50/50 border-emerald-200"
        >
          <Statistic
            title={
              <Text
                style={{ fontSize: "11px", fontWeight: 700, color: "#047857" }}
              >
                ✅ DELIVERED
              </Text>
            }
            ...
            styles={{
              content: { fontWeight: 900, fontSize: "22px", color: "#059669" },
            }}
          />
        </Card>
      </div>
      ...
      <Card
        style={{ borderRadius: 16 }}
        styles={{ body: { padding: "12px 16px 12px 16px" } }}
        className="shadow-sm"
      >
        ...
        <Input
          prefix={
            <SearchOutlined style={{ color: "#94a3b8", marginRight: 4 }} />
          }
                        ...
                        <Popover
                          ...
                          content={
                            <FollowUpPanel
                              ...
                              compact
                              onAdded={(entry) =>
                                handleFollowUpAdded(item._id, entry)
                              }
                            />
                          }
                        >
                        ...
                        <div className="flex items-center justify-start gap-1.5 mt-0.5">
                          ...
                          <Tooltip title="Copy Consignment ID">
                            <Button
                              ...
                              onClick={() =>
                                copyToClipboard(item.courierTrackingId!)
                              }
