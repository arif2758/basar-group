...
export async function POST(req: Request) {
  try {
    ...
    if (
      !inputRawText ||
      typeof inputRawText !== "string" ||
      inputRawText.trim().length === 0
    ) {
...
- Do NOT output markdown codeblocks, output raw JSON only.`;
        ...
        console.error(
          "Gemini API error, falling back to regex parser:",
          geminiError,
        );
      }
    }
    ...
    const rawItemsList =
      Array.isArray(parsedData.items) && parsedData.items.length > 0
        ...
        : [
            {
              productQuery: parsedData.productQuery || rawText,
    ...
    for (const rawItem of rawItemsList) {
      ...
      const matched = storeProducts.find((p: any) => {
        ...
        return (
          ...
          q
            .split(/\s+/)
            .some(
              (word: string) => word.length > 3 && titleLower.includes(word),
            )
        );
      });
      ...
      parsedItems.push({
        ...
        color:
          rawItem.color ||
          (matchedProductColors.length > 0 ? matchedProductColors[0] : ""),
        size: rawItem.size || "",
...
function fallbackRegexParse(text: string) {
  ...
  const areaMatch = text.match(
    /(?:ঠিকানা|এরিয়া|গ্রাম|বাসা|রোড)\s*[:=\-ঃ]\s*([^\n\r,]+)/i,
  );
  const upazilaMatch = text.match(
    /(?:উপজেলা|থানা|upazila|thana)\s*[:=\-ঃ]\s*([^\n\r,]+)/i,
  );
  const districtMatch = text.match(
    /(?:জেলা|জেল|zila|district)\s*[:=\-ঃ]\s*([^\n\r,]+)/i,
  );

  const isOutside =
    /(?:মুন্সীগঞ্জ|মানিকগঞ্জ|গাজীপুর|সাভার|নারায়ণগঞ্জ|সিংগার|চট্টগ্রাম|সিলেট|রাজশাহী|খুলনা|বরিশাল|রংপুর|কুমিল্লা|outside|osd)/i.test(
      text,
    ) ||
    (districtMatch && !/ঢাকা|dhaka/i.test(districtMatch[1]));

  const addressLine1 = areaMatch
    ? areaMatch[1].trim()
    : name || phone
      ? ""
      : text.slice(0, 50);
  ...
  if (!name) {
    ...
    for (const l of lines) {
      if (
        !l.match(/01[3-9]\d{8}/) &&
        !l.match(/(?:গ্রাম|উপজেলা|জেলা|জেল|ঠিকানা|মোবাইল)/i)
      ) {
