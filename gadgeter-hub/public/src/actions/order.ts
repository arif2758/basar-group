...
export async function createOrder(formData: FormData) {
  ...
  const channelSource =
    (formData.get("channelSource") as ChannelSource) || "web";
  ...
  const deliveryZone =
    DELIVERY_ZONES[data.deliveryArea as DeliveryZone]?.badgeLabel ||
    "ISD (Inside Dhaka)";
...
export async function createAdminManualOrder(data: AdminManualOrderInput) {
  ...
  const courierCodFee =
    data.paymentMethod === "cod" ? Math.round(codCollectionAmount * 0.01) : 0;
  const deliveryZone =
    data.deliveryArea === "custom"
...
export async function updateAdminOrder(data: UpdateAdminOrderInput) {
  ...
  const deliveryArea =
    data.deliveryArea || order.shipping.deliveryArea || "dhaka";
  const shippingName =
    data.isGift && data.receiverName
      ? data.receiverName
      : data.name || order.shipping.name;
  const shippingPhone =
    data.isGift && data.receiverPhone
      ? data.receiverPhone
  ...
  if (data.customerNotes !== undefined)
    order.customerNotes = data.customerNotes;
  ...
  order.shipping = {
    ...
    district:
      data.district !== undefined ? data.district : order.shipping.district,
    deliveryArea,
    deliveryZone:
      DELIVERY_ZONES[deliveryArea as DeliveryZone]?.badgeLabel ||
      order.shipping.deliveryZone,
  };

  // 3. Recalculate Shipping Cost & Total
  const shippingCost =
    data.shippingCostOverride !== undefined && data.shippingCostOverride >= 0
      ? data.shippingCostOverride
      : calculateShippingCost(
          deliveryArea as DeliveryZone,
          order.totalWeightGrams || 500,
        );
...
export async function lookupCustomerHistory(phone: string) {
  ...
  const pastOrders = await Order.find({
    ...
    .select(
      "orderNumber orderStatus shipping subtotal shippingCost total createdAt",
    )
    .sort({ createdAt: -1 })
    .lean();
