...
export async function bookSingleOrderAction(
  ...
  try {
    ...
    if (!order) {
      return {
        success: false,
        error: "অর্ডার খুঁজে পাওয়া যায়নি (Order not found)",
      };
    }
  ...
  try {
    ...
    if (syncRes.status) {
      ...
      order.courierRiderName =
        syncRes.riderName || order.courierRiderName || "";
      order.courierRiderPhone =
        syncRes.riderPhone || order.courierRiderPhone || "";
      order.courierLastLogDesc =
        syncRes.lastLogDesc || order.courierLastLogDesc || "";
