...
export async function getPublicPathaoTracking(
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        error: "Pathao returned non-JSON: " + text.slice(0, 100),
      };
    }
  ...
  try {
    ...
    if (cleanId) {
      ...
      if (tracking.success && tracking.data) {
        ...
        order.courierLastUpdated = new Date();
        console.warn(
          "[Pathao] tracking fetch failed during save:",
          tracking.error,
        );
      }
    } else {
    ...
    return {
      success: false,
      error: err.message || "Failed to update order tracking ID",
    };
  }
}
