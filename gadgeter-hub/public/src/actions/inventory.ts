  ...
  try {
    ...
    activeOrders.forEach((order: any) => {
      ...
      items.forEach((item: any) => {
        ...
        const qty = Math.max(
          1,
          Number(item.quantity || item.itemQuantity || 1),
        );
    ...
    return {
      success: false,
      error: err.message || "Failed to generate pick list",
    };
  }
}
...
export async function getInventoryMatrixAction(): Promise<{
  ...
  try {
    ...
    const products = await Product.find({})
      ...
      .select(
        "title sku thumbnail stockQuantity variants colors sizes category status",
      )
      .lean();
    ...
    const matrix: InventoryMatrixItem[] = products.map((p: any) => {
      ...
      const rawColors: string[] =
        Array.isArray(p.colors) && p.colors.length > 0 ? p.colors : [];
      ...
      let variantsList = hasVariants
        ? p.variants.map((v: any, vIdx: number) => {
            ...
            return {
              ...
              color:
                colorAttr ||
                (rawColors.includes(v.variantTitle)
                  ? v.variantTitle
                  : undefined),
      ...
      if (hasVariants) {
        totalStock = variantsList.reduce(
    ...
    return {
      success: false,
      error: err.message || "Failed to load inventory matrix",
    };
  }
}
  ...
  try {
    ...
    if (variantId) {
      ...
      const updatedProduct =
        await Product.findById(productId).select("variants");
