...
export async function getFinancialAnalyticsAction(range: {
  ...
  const products = await Product.find({})
    .select(
      "_id title sku thumbnail costPrice salePrice regularPrice targetAdCost",
    )
    .lean();
  ...
  const productSalesMap = new Map<
    string,
    { unitsSold: number; revenue: number; cogs: number; estDelivery: number }
  >();
  ...
  orders.forEach((order: any) => {
    ...
    // 1% COD Fee
    const codFee =
      order.courierCodFee !== undefined && order.courierCodFee >= 0
    ...
    items.forEach((item: any) => {
        ...
        if (productTitleMap.has(rawTitle)) {
          ...
          for (const [t, id] of productTitleMap.entries()) {
            if (t.includes("m3-t") && rawTitle.includes("m3-t")) {
              ...
              !rawTitle.includes("m3-t") &&
              !t.includes("m3-t") &&
              (t.includes(rawTitle) || rawTitle.includes(t))
            ) {
              pId = id;
              break;
            }
      ...
      if (
        !pId &&
        item.productSku &&
        productTitleMap.has(item.productSku.trim().toLowerCase())
      ) {
  ...
  const netProfit =
    totalRevenue - totalCogs - totalCourierExpenses - totalLoggedExpenses;
  ...
  const totalInvestment =
    totalCogs + totalCourierExpenses + totalLoggedExpenses;
  const roiPercent =
    totalInvestment > 0
  ...
  const summary: FinancialSummary = {
    dateRangeLabel: range?.startDate
      ? `${range.startDate} to ${range.endDate || "Today"}`
      : "All Time",
  ...
  products.forEach((p: any) => {
    ...
    const sales = productSalesMap.get(pId) || {
      unitsSold: 0,
      revenue: 0,
      cogs: 0,
      estDelivery: 0,
    ...
    const estTargetAd =
      loggedAd > 0 ? loggedAd : pInfo.targetAdCost * sales.unitsSold;
    const attributedAd = estTargetAd;

    const prodNetProfit = sales.revenue - sales.cogs - attributedAd;
    const prodMargin =
      sales.revenue > 0 ? (prodNetProfit / sales.revenue) * 100 : 0;
    ...
    const universalScore =
      sales.unitsSold > 0
        ? prodNetProfit > 0
          ? Math.round(
              prodNetProfit *
                Math.sqrt(sales.unitsSold) *
                (1 + Math.max(0, prodRoi) / 100),
            )
          : prodNetProfit
        : -999999;
  ...
  if (
    ...
    (winningProducts[0].netProfit > 0 || winningProducts[0].unitsSold > 0)
  ) {
